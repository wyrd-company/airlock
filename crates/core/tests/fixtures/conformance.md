...
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------ |
| `REPO-ORG-01`  | The repository is in the org its audience implies: `boblangley` if overfit to Bob, `wyrd-company` if a stranger benefits, `flapstack` if a vendor-neutral public good | Observation | Judgment                                                           |
| `REPO-ORG-02`  | A `boblangley` repository that has generalized has been considered for graduation to `wyrd-company`                                                                   | Observation | Judgment                                                           |
| `REPO-NAME-01` | The name is `lower-kebab-case`; no underscores, camelCase, or PascalCase                                                                                              | Required    | Repository name                                                    |
| `REPO-NAME-02` | A dotted name is used only where the repository is a deployable site and the name is its hostname                                                                     | Required    | Repository name                                                    |
| `REPO-NAME-03` | A repository in a product family carries the family prefix                                                                                                            | Observation | Repository name                                                    |
| `REPO-NAME-04` | The family prefix and the family topic agree                                                                                                                          | Required    | Name vs topics                                                     |
...
| `REPO-META-03` | The description expands any acronym on first use                                                                                                                      | Required    | Read it                                                            |
| `REPO-META-04` | The description contains no self-deprecation                                                                                                                          | Required    | Read it                                                            |
| `REPO-META-05` | The description agrees with the README's opening line                                                                                                                 | Required    | Compare                                                            |
...
| `REPO-META-08` | A repository in a product family carries the family topic                                                                                                             | Required    | Compare against `topics.md`                                        |
...
| `REPO-LIC-02` | The license matches the repository's category: Apache-2.0 by default, CC0-1.0 for specs and schemas, MIT for plumbing, or a matched ecosystem license | Required    | Judgment against the category table          |
...
| `REPO-LIC-05` | A plumbing repository's README states that the license covers packaging, not payload                                                                  | Required    | Read the README                              |
| `REPO-LIC-06` | A non-default license choice has a stated reason in the README or a decision record                                                                   | Observation | Read                                         |
...
| `REPO-FILE-15` | Community health files are inherited from the org rather than duplicated, unless the local copy deliberately differs | Observation | Compare against the org `.github` repository |

## README

| Id               | Assertion                                                                  | Severity    | How to check                       |
| ---------------- | -------------------------------------------------------------------------- | ----------- | ---------------------------------- |
| `REPO-README-01` | Opens with a title and a one-line statement of what it is                  | Required    | Read                               |
| `REPO-README-02` | Badges are present                                                         | Required    | Read                               |
| `REPO-README-03` | An installation section covers every supported channel                     | Blocking    | Compare against published channels |
| `REPO-README-04` | A quickstart or usage section contains at least one real, runnable example | Blocking    | Read                               |
| `REPO-README-05` | A repository with a user interface has an animated demo                    | Observation | Read                               |
| `REPO-README-06` | Where a demo exists, its `.tape` source is committed                       | Required    | File presence                      |
| `REPO-README-07` | Prerequisites appear only where non-obvious                                | Observation | Read                               |
| `REPO-README-08` | No license section                                                         | Required    | Read                               |
| `REPO-README-09` | No contributing section                                                    | Required    | Read                               |
...
| `REPO-GIT-14` | No secret or variable is named after the task that introduced it                                                            | Required    | Workflow source                                       |
...
| `REPO-TASK-02` | Applicability of `build` and `dev` was an explicit decision, and they exist where applicable     | Required | `task --list-all` plus the recorded decision |
| `REPO-TASK-03` | `task check` runs everything CI runs                                                             | Required | Compare taskfile against workflows           |
...
| `REPO-CI-10`   | No workflow mutates repository settings outside the reconcile workflow                           | Required | Workflow contents                            |
...
| `REPO-CD-05`   | A CD `workflow_dispatch` can only re-deliver an existing tag, never create a release             | Blocking | Dispatch inputs and their use                |
| `REPO-CD-06`   | CD asserts every required publication credential is present before doing any work                | Required | Workflow contents                            |
...
| Id              | Assertion                                                                                                                                 | Severity | How to check  |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------- |
| `REPO-AGENT-01` | `AGENTS.md` covers what the repository is, commands by reference, repository-specific conventions, what not to touch, and where docs live | Required | Read          |
| `REPO-AGENT-02` | `AGENTS.md` contains nothing discoverable by reading the repository                                                                       | Required | Read          |
| `REPO-AGENT-03` | `AGENTS.md` does not restate the README or `CONTRIBUTING.md`                                                                              | Required | Compare       |
| `REPO-AGENT-04` | `CONTRIBUTING.md` contains no command sequences beyond `task check`                                                                       | Required | Read          |
| `REPO-AGENT-05` | Generated directories are marked `linguist-generated=true` in `.gitattributes`                                                            | Required | File contents |

## Documentation

| Id             | Assertion                                                                                    | Severity    | How to check                             |
| -------------- | -------------------------------------------------------------------------------------------- | ----------- | ---------------------------------------- |
| `REPO-DOCS-01` | User-facing pages sit at the top level of `docs/`                                            | Observation | Directory listing                        |
...
| `REPO-REL-05` | The release workflow asserts the pinned SHA equals current `main`                | Blocking | Workflow contents                         |
| `REPO-REL-06` | The release workflow asserts a successful push-event CI run for exactly that SHA | Blocking | Workflow contents                         |
